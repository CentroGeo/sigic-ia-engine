...
def optimized_rag_search(
    context_id: int, query: str, top_k: int = 50
    ...
    try:
        ...
        if query_embedding is None or len(query_embedding) == 0:
            print(
                f"[WARNING] No se pudo generar embedding para la consulta: {query[:100]}..."
            )
            return []
        ...
        ).annotate(similarity=1 - L2Distance("embedding", query_embedding))
        ...
        if query_language in ["es", "en", "fr"]:
            ...
            if language_chunks.exists():
                ...
                print(
                    f"[DEBUG] No hay chunks en {query_language}, usando todos los idiomas"
                )
        ...
        print(
            f"[DEBUG] RAG search: {len(filtered_chunks)} chunks encontrados para query en {query_language}"
        )
        print(
            f"[DEBUG] Similitudes: {[round(chunk.similarity, 3) for chunk in filtered_chunks[:5]]}"
        )

        return filtered_chunks[
            : min(20, len(filtered_chunks))
        ]  # Limitar a 20 mejores resultados
...
def chat(request):
    ...
    if "type" not in payload or payload["type"] not in ["Preguntar", "RAG"]:
        return JsonResponse(
            {"error": "El parámetro 'type' debe ser 'Preguntar' o 'RAG'"}, status=400
        )

    if payload["type"] == "RAG" and "context_id" not in payload:
        return JsonResponse(
            {"error": "Se requiere context_id para tipo RAG"}, status=400
        )
    ...
    if not acquired:
        return JsonResponse(
            {"error": "Servicio ocupado, intenta más tarde"}, status=503
        )
                ...
                if relevant_chunks:
                    ...
                    for doc_name, chunks in docs_context.items():
                        ...
                        for i, chunk_data in enumerate(
                            chunks[:3]
                        ):  # Max 3 chunks por documento
                            rag_context += f"- {chunk_data['text']}\n"
                        rag_context += "\n"

                    print(
                        f"[DEBUG] RAG context construido: {len(rag_context)} caracteres"
                    )
...
- Sé conciso pero completo en tu respuesta"""
                    ...
                    updated_payload["messages"].insert(
                    ...
                    print(
                        "[WARNING] No se encontraron chunks relevantes para la consulta RAG"
                    )
            ...
            # =================== LLAMADA A OLLAMA ===================
            print(
                f"[DEBUG] Enviando {len(updated_payload['messages'])} mensajes a Ollama"
            )
            ...
            cleaned_messages = [
                msg for msg in new_messages if msg.get("role") != "system"
            ]
            ...
            if update_history.title is None:
                ...
                generated_title = generate_chat_title(
                    server, first_question, first_answer, model
                )
                if generated_title:
    ...
    return StreamingHttpResponse(
        event_stream(payload), content_type="text/event-stream"
    )
...
def get_chat_histories(request):
    ...
    histories = histories.prefetch_related("context__workspace").order_by(
        "-credate_date"
    )
...
def generate_chat_title(
    server_url: str, question: str, answer: str, model_name: str
) -> str:
    """
