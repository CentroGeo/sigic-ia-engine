...
def upload_file_to_geonode(file, token, cookie=None, title="Sin título"):
    files = {"doc_file": (file.name, file, file.content_type)}
    data = {"title": title}
    ...
    if "archivos" in request.FILES:
        fs = FileSystemStorage(
            location=os.path.join(
                settings.MEDIA_ROOT, "uploads/proyectos", str(workspace.id)
            )
        )
        ...
        for uploaded_file in request.FILES.getlist("archivos"):
            ...
            # try:
            # geo_response = upload_file_to_geonode(
            #     uploaded_file, token, cookie, filename
            # )
